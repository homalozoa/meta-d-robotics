...
| USB storage and HID | Generic USB mass storage, UAS, keyboard and mouse | Kernel support is built in and manual mounting is available; `evdev` diagnostics and privileged desktop-style automount are intentionally absent for now | Read/write/sync/unmount a disposable filesystem; keyboard/mouse input; suspend with no mounted removable media |
...
| Sleep button and suspend | DT `gpio-keys` wake key plus the RDKOS suspend listener/hook | Kernel suspend and wake-key DT contract are present; the `evdev` package, event-device discovery, and headless-safe audio/Wi-Fi/network quiesce policy remain pending | Discover the key by identity rather than `event0`; repeated suspend/resume with Ethernet, Wi-Fi, audio, USB and BPU idle/active |
...
3. Add the sleep-button event package and a headless-safe suspend/resume policy
   after identifying the live input device and testing driver quiesce order.
