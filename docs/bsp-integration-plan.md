...
| USB storage and HID | Generic USB mass storage, UAS, keyboard and mouse | Kernel support, `evdev`, and `evtest` are included; manual mounting is available and privileged desktop-style automount remains intentionally absent | Read/write/sync/unmount a disposable filesystem; keyboard/mouse input; suspend with no mounted removable media |
...
| Core board peripherals | HPU3501 RTC, ACT LED, GPIO power key, TCAN4x5x, DT-declared SPI, I2C/GPIO tools | Implemented; input event handler and diagnostics included without an automatic suspend action | RTC read, ACT trigger, identify the GPIO key event node, SPI nodes, I2C enumeration, CAN loopback without external traffic |
...
| Sleep button and suspend | DT `gpio-keys` wake key plus the RDKOS suspend listener/hook | Kernel suspend, wake-key DT contract, `evdev`, and `evtest` are present; identity-based event discovery and headless-safe audio/Wi-Fi/network quiesce policy remain pending | Discover the key by identity rather than `event0`; repeated suspend/resume with Ethernet, Wi-Fi, audio, USB and BPU idle/active |
...
3. Add identity-based sleep-button discovery and a headless-safe
   suspend/resume policy after testing driver quiesce order.
