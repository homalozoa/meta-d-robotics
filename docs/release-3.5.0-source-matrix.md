...
| `hobot-multimedia`, `hobot-multimedia-loader`, and `hobot-multimedia-gpu` | `3.0.5` | `RDK_X5_SRCREV_HOBOT_MULTIMEDIA` | `Apache-2.0`; `LICENSE` `3b83ef96387f14655fc854ddc3c6bd57` | `rdk-x5` only.  `libdir` keeps the audited vendor ABI in `/usr/hobot/lib`; the GPU split alone also permits `dev-so` because Vivante EGL dlopens the official bare EGL/GLES2 symlinks at runtime and fails with equivalent hard links.  cJSON and libdrm use Wrynose providers and all other binary QA remains enabled.  The GPU split is runtime-only, conflicts with Mesa's same-SONAME packages, and excludes unreviewed OpenCL/Vulkan/GLES1 components. |
| `hobot-multimedia-headers` | `3.0.4` | `RDK_X5_SRCREV_HOBOT_MULTIMEDIA_DEV` | `Apache-2.0 & MIT`; repository `LICENSE` `3b83ef96387f14655fc854ddc3c6bd57`, GLES2 notice `dead6c11b351746950831f19bad48bd3` | `rdk-x5` only.  Installs only EGL, GLES2, GBM, KHR, and GC820/Nano2D headers into the private `/usr/hobot` recipe sysroot.  It is not installed in the image and does not claim a global graphics provider. |
...
| `RDK_X5_SRCREV_HOBOT_MULTIMEDIA_DEV` | `x5-hobot-multimedia-dev` | `ac255ab4a0d1f7fc485e2b1833b6428229c8e1bb` | `hobot-camera` build headers; selected `hobot-multimedia-headers` graphics API |
