...
| `hobot-multimedia`, `hobot-multimedia-loader`, and `hobot-multimedia-gpu` | `3.0.5` | `RDK_X5_SRCREV_HOBOT_MULTIMEDIA` | `Apache-2.0`; `LICENSE` `3b83ef96387f14655fc854ddc3c6bd57` | `rdk-x5` only.  `libdir` is the sole QA exception because the audited vendor ABI must remain in `/usr/hobot/lib`; cJSON and libdrm use Wrynose providers and all other binary QA remains enabled.  The vendor-required bare EGL/GLES2 runtime names are hard links to the audited versioned payloads, avoiding a `dev-so` exception.  The GPU split is runtime-only, conflicts with Mesa's same-SONAME packages, and excludes unreviewed OpenCL/Vulkan/GLES1 components. |
